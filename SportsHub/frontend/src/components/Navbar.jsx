  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-20 w-full">
          ...
          <Link to="/" className="flex items-center gap-x-2 min-w-0">
            <div className={`relative w-10 h-10 rounded-2xl flex items-center justify-center transition-all duration-300 ${isDarkMode
            ...
            <div className="min-w-0">
              <h1 className={`text-xl font-bold truncate max-w-[110px] sm:max-w-[150px] ${isDarkMode ? 'text-white' : 'text-gray-900'
                }`}>
                Sports<span className={isDarkMode ? 'text-orange-400' : 'text-blue-500'}>Hub</span>
              </h1>
              <p className={`text-xs truncate max-w-[100px] sm:max-w-[140px] ${isDarkMode ? 'text-gray-400' : 'text-gray-600'
                }`}>
          ...
          <div className="hidden lg:flex items-center gap-x-1 flex-wrap min-w-0 overflow-x-auto">
            ...
            {navItems
              ...
              .map((item) => (
                <Link
                  key={item.label}
                  to={item.href}
                  className={`flex items-center gap-x-1 px-3 py-2 rounded-xl font-medium transition-all duration-300 group relative truncate max-w-[110px] ${isActive(item.href)
                    ? isDarkMode
                ...
                >
                  ...
                  {isActive(item.href) && (
                    <motion.div
                      layoutId="activeTab"
                      className={`absolute inset-0 rounded-xl ${isDarkMode ? 'bg-orange-500/10' : 'bg-blue-500/10'
                        }`}
                      style={{ zIndex: -1 }}
                    />
                  )}
            ...
            {isLoggedIn ? (
              <Link
                to="/logout"
                className={`flex items-center px-3 py-2 rounded-xl transition-all duration-300 group relative ${isDarkMode
              ...
              <Link
                to="/login"
                className={`flex items-center gap-x-1 px-3 py-2 rounded-xl font-medium transition-all duration-300 group relative ${isDarkMode
                  ? 'text-gray-300 hover:bg-white/10 hover:text-orange-400'
                  : 'text-gray-700 hover:bg-black/10 hover:text-blue-600'
                  }`}
                style={{ minWidth: 0 }}
      ...
      <motion.div
        ...
        className={`lg:hidden overflow-hidden ${isDarkMode
        ...
        <div className="px-4 py-4 space-y-2">
          {navItems.map((item) => (
            <Link
              key={item.label}
              to={item.href}
              className={`flex items-center space-x-3 px-4 py-3 rounded-xl font-medium transition-all duration-300 ${isActive(item.href)
                ? isDarkMode
