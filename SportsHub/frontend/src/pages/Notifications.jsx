...
const Notifications = ({ isDarkMode, userType, isAdmin }) => {
  ...
  useEffect(() => {
    ...
    const fetchPendingClubs = async () => {
      ...
      try {
        const res = await axios.get(`${import.meta.env.BACKEND_URL}/clubs/pending-clubs`, { withCredentials: true });
                ...
                {pendingClubs.map(club => (
                  <div key={club._id} className={`rounded-xl p-6 shadow ${isDarkMode ? 'bg-white/10' : 'bg-black/10'}`}>
                    ...
                    <button
                      onClick={async () => {
                        try {
                          await axios.patch(`${import.meta.env.BACKEND_URL}/clubs/${club._id}/approve`, {}, { withCredentials: true });
