...
const Live = ({ isDarkMode }) => {
    ...
    const fetchMatches = async () => {
     try {
       const [liveRes, upcomingRes, pastRes] = await Promise.all([
         axios.get(`${import.meta.env.VITE_BACKEND_URL}/match/live`, { withCredentials: true }),
  ...
  const handleBroadcastClick = async () => {
    if (isLoggedIn && userType === 'club') {
      try {
        const res = await axios.get(`${import.meta.env.VITE_BACKEND_URL}/api/club/my-club`, { withCredentials: true });
        if (res.status === 200) {
